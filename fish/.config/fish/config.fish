...
if status is-interactive
  ...
  function rd --description "recent directories"
    set -l dir (tac $CDHISTFILE | awk '$0 != ENVIRON["PWD"] && !seen[$0]++' | $FZZYPICKER --p='Recent directories:')
  ...
  function rf --description "recent files"
    while true
      set -l file (tac $EDHISTFILE | awk '!seen[$0]++' | $FZZYPICKER --p='Recent files:')
  ...
  function bf --description "browse files"
    while true
      set -l file (fd --hidden --color=never --type f --exclude .git -x stat -c='%Y %n' | sort -rn | cut -d' ' -f2- | sed 's|^\./||' | $FZZYPICKER --p='Files:')
  ...
  function of --description "obliterate file"
    while true
      set -l file (fd --hidden --color=never --type f --exclude .git -x stat -c='%Y %n' | sort -rn | cut -d' ' -f2- | sed 's|^\./||' | $FZZYPICKER --p='Obliterate Files:')
  ...
  function mf --description "move file"
    while true
      set -l file (fd --hidden --color=never --type f --exclude .git -x stat -c='%Y %n' | sort -rn | cut -d' ' -f2- | sed 's|^\./||' | $FZZYPICKER --p='Rename Files:')
  ...
  # TODO: Use realpath instead of regular so I can use it inside subdirectories.
  function gf --description "git files"
    while true
      set -l file (git status --porcelain | awk '{print $2}' | xargs -r stat -c='%Y %n' 2>/dev/null | sort -rn | cut -d' ' -f2- | $FZZYPICKER --lines=15 --p='Git Changes:')
  ...
  function ga --description "git add file"
    while true
      set -l file (git status --porcelain | awk '{print $2}' | xargs -r stat -c='%Y %n' 2>/dev/null | sort -rn | cut -d' ' -f2- | $FZZYPICKER --lines=15 --p='Stage File:')
