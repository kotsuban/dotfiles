...
" CompilerSet makeprg=pytest (with virtualenv support)
let s:pytest_exe = expand('.venv/bin/pytest')

if has('unix')
  execute $'CompilerSet makeprg=/usr/bin/env\ PYTHONWARNINGS=ignore\ '.s:pytest_exe.'\ '.escape(get(b:, 'pytest_makeprg_params', get(g:, 'pytest_makeprg_params', '--tb=short --quiet')), ' \|"')
elseif has('win32')
  " Adjust for Windows if needed (probably not .venv/bin)
  ...
  execute $'CompilerSet makeprg='.s:pytest_exe.'\ '.escape(get(b:, 'pytest_makeprg_params', get(g:, 'pytest_makeprg_params', '--tb=short --quiet')), ' \|"')
endif
